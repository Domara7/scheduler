...
it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
  ...
  fireEvent.change(getByTestId(appointment, "student-name-input"), {
    target: { value: "Marcus Zimmer" },
  });
  fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));
  fireEvent.click(getByText(appointment, "Save"));

  //5. Check that the text "Saving" is displayed.
  expect(getByText(appointment, "Saving")).toBeInTheDocument();
  await waitForElement(() => getByText(appointment, "Saving"));

  //6. Check that the DayListItem with the text "Monday" still has the text "1 spot remaining"
  const day = getAllByTestId(container, "day").find((day) =>
    queryByText(day, "Monday")
  );
